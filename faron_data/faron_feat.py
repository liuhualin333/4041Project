...
TRAIN_FILENAME = "./faron_feat_train.csv"
TEST_FILENAME = "./faron_feat_test.csv"
...
train_test['magic3'] = train_test[ID_COLUMN].diff().fillna(9999999).astype(int) # Id based features, get the difference of the job id in starttime order
train_test['magic4'] = train_test[ID_COLUMN].iloc[::-1].diff().fillna(9999999).astype(int) # Id based features, get the difference of the job id in starttime descending order

train_test = train_test.sort_values(by=['index']).drop(['index'], axis=1)
train = train_test.iloc[:ntrain, :]
test = train_test.iloc[ntrain:, :]

# features = np.setdiff1d(list(train.columns), [TARGET_COLUMN, ID_COLUMN]) # diffrence between two arrays, extract features
